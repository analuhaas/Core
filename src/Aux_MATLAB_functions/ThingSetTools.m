...
classdef ThingSetTools < handle
    ...
    methods
        function obj = ThingSetTools(port, baudRate, timeoutSeconds, vid, pid, verbose)
            % Connect to a ThingSet-over-shell device. `port` may be:
            %   - "" (default): candidates are found by USB `vid`/`pid`
            %     (see findPorts), falling back to every serial port on
            %     the system if none match via serialportlist(). Both of
            %     those enumeration steps query every serial port on the
            %     system - including virtual ones (e.g. "Standard Serial
            %     over Bluetooth link") that Windows can take tens of
            %     seconds to respond about - so auto-detection can be
            %     slow on a machine with several such ports.
            %   - a single port, e.g. "COM39": tried directly.
            %   - a string array of ports, e.g. ["COM39" "COM40"]: tried
            %     in order, same as auto-detected candidates. Use this to
            %     skip enumeration entirely once you know which port(s)
            %     the board can show up on - see the USB\VID_2FE3 entries
            %     under Win32_PnPEntity (or Device Manager) to find them.
            % Each candidate is tried in turn until one answers the
            % ThingSet handshake.
            %
            ...
            port = port(strlength(port) > 0);
            if ~isempty(port)
                candidates = port;
                obj.log("explicit candidate port(s) given, skipping auto-detection: %s", ...
                    strjoin(candidates, ", "));
            else
        ...
        function tf = isOpen(obj)
            % True if this object holds a live serial connection. isvalid(obj)
            % alone isn't enough to tell: close() releases the underlying
            % serial port but the ThingSetTools object itself remains a
            % valid (just disconnected) handle.
            tf = ~isempty(obj.Serial);
        end
        ...
        function releaseSerial(obj)
            ...
            if ~isempty(obj.Serial)
                try
                    ...
                    % already gone (e.g. device unplugged) - nothing to do
                end
            end
            obj.Serial = [];
        end
        ...
        function connect(obj, port, baudRate, timeoutSeconds)
            ...
            for attempt = 1:maxAttempts
                try
                    ...
                    obj.log("open %s failed (attempt %d/%d): %s - retrying in 1.5s", ...
                        port, attempt, maxAttempts, ME.message);
                    pause(1.5);
                end
            end
            pause(0.3);
